...
export function InputBar({ onSubmit, disabled }: Props) {
  ...
  const handleTextareaContentChange = useCallback(() => {
    ...
    if (text.length - previousTextLengthRef.current > PASTE_CHAR_THRESHOLD) {
      const cursorPos = textarea.cursorOffset;
      const addedLength = text.length - previousTextLengthRef.current;
      const pasteStart = cursorPos - addedLength;
      const pastedContent = text.slice(pasteStart, cursorPos);
  ...
  const handleSubmit = useCallback(() => {
    ...
    const chips = pasteChips;
    setPasteChips([]);

    const pasted = chips.map((c) => c.content).join("\n\n");
    const combined = pasted ? (text ? `${pasted}\n\n${text}` : pasted) : text;
